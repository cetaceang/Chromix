...
def source_version(repo: str, sha: str, platform: str | None = None) -> str:
    ...
    filename = f"CHROMIUM_{platform.upper()}_VERSION" if platform else "CHROMIUM_VERSION"
    try:
        ...
        if (platform is None or error.returncode != 1 or not isinstance(error.stderr, str)
