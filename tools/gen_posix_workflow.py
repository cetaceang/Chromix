...
def run_step(stage: int) -> str:
    ...
    return (
        ...
        env:
          ...
          # Mac runtime gates need 45m in addition to packaging/checkpoint reserve.
          CHROMIX_RESERVE_MINUTES: ${{ inputs.platform == 'macos' && '90' || inputs['max-stages'] == 1 && '15' || '45' }}
...
FINAL_UPLOADS = """      - name: Upload final bundle
      ...
      - name: Upload fingerprint source receipt
        if: steps.stage.outputs.finished == 'true'
        uses: actions/upload-artifact@v4
        with:
          name: ${{ inputs.artifact }}-fingerprint-source
          path: ${{ runner.temp }}/chromix-build/fingerprint-diagnostics/source-final.json
          if-no-files-found: error
          retention-days: 14
"""
...
""" + PIN_VERSION + """      - name: Install runtime libraries
      ...
      - name: Download same-run source verification receipt
        ...
        with:
          name: ${{ inputs.artifact }}-fingerprint-source
          path: ${{ runner.temp }}/chromix-native-source
      - name: Native ARM64 fingerprint regression gate
        run: |
          set -euo pipefail
      ...
      - name: Upload native verification diagnostics
        ...
        with:
          ...
          path: |
            ${{ runner.temp }}/chromix-linux-arm64-native-smoke.log
            ${{ runner.temp }}/chromix-native-fingerprint/
          if-no-files-found: warn
          retention-days: 14
"""
