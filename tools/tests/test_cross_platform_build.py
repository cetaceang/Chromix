...
class CrossPlatformBuildRegressionTest(unittest.TestCase):
        ...
        for name, job in workflow["jobs"].items():
            with self.subTest(job=name):
                ...
                if name.startswith("posix-"):
                    ...
                    for pin in ("CHROMIUM_VERSION", "CHROMIUM_LINUX_VERSION", "tools/platform_pins.py",
    ...
    def test_linux_entrypoints_trigger_on_independent_pins(self):
        import yaml

        for arch in ("x64", "arm64"):
            workflow = yaml.safe_load((WORKFLOWS / f"build-linux-{arch}.yml").read_text())
            paths = workflow[True]["push"]["paths"]
            self.assertIn("CHROMIUM_LINUX_VERSION", paths)
