...
def windows153_source(repo):
    ...
    pins = cache.load_pins(repo, "macos")
...
class FetchUpstreamCacheTest(unittest.TestCase):
    def setUp(self):
        ...
        for name in ("CHROMIUM_VERSION", "CHROMIUM_LINUX_VERSION", "CHROMIUM_WINDOWS_VERSION",
    ...
    def linux_overrides(self):
        pins = cache.load_pins(self.root, "macos")
    ...
    def test_linux_old_global_manifest_cannot_authorize_new_version(self):
        self.linux_overrides()
        for field in ("chromium_version", "ungoogled_commit"):
    ...
    def test_disabled_windows_fixture_refuses_network_before_donor_validation(self):
        ...
        self.assertEqual(manifest["chromium_version"], "152.0.7977.82")
        for arch in ("x64", "arm64"):
    ...
    def test_windows_override_cannot_fall_back_to_shared_152_identity(self):
        self.manifest["sources"]["windows"] = windows153_source(self.root)
        self.save_manifest()
        for field in ("chromium_version", "ungoogled_commit"):
            with self.subTest(field=field):
