"""Platform pin selection is fail-closed without changing legacy identities."""
import json
...
def repository(root, *, linux=False, windows=False):
    values = {
...
@pytest.mark.parametrize("platform,version,core,overlay", [
    ...
    ("windows", GLOBAL_VERSION, "e71b91c6e336d0f25cfc6b9ef09298a9d2506e24",
     "333bc7dfff72ff4abc4d9cc76bc41de300a46e06"),
])
def test_checked_in_target_pins(platform, version, core, overlay):
    ...
    assert (pins.ROOT / "CHROMIUM_VERSION").read_text().strip() == GLOBAL_VERSION
    assert (pins.ROOT / "UNGOOGLED_VERSION").read_text().strip() == GLOBAL_VERSION + "-1"
    assert (pins.ROOT / "UNGOOGLED_WINDOWS_VERSION").read_text().strip() == GLOBAL_VERSION + "-1.1"
...
def test_nonliteral_override_cannot_be_treated_as_absent(tmp_path, value):
    repository(tmp_path)
    path = tmp_path / "build/ungoogled-revisions.psd1"
    path.write_text(path.read_text().replace("}\n", f"  LinuxChromiumVersion = {value}\n}}\n"))
    with pytest.raises(pins.PinError):
        pins.load_pins(tmp_path, "linux")


def test_duplicate_required_pin_is_rejected(tmp_path):
    repository(tmp_path, linux=True)
    path = tmp_path / "build/ungoogled-revisions.psd1"
    path.write_text(path.read_text().replace("}\n", f'  LinuxUngoogledCommit = "{"e" * 40}"\n}}\n'))
...
def test_json_cli_emits_validated_effective_pins(tmp_path, platform):
    repository(tmp_path, linux=True, windows=True)
