...
HISTORY_PATH = Path(__file__).with_name("fixtures") / "windows152_migration_history.json"
HISTORY_SHA256 = "974298ee6fff14dcc610111672008d0e5ad8be058a00ced598c55bf53f12b165"
...
def load_history(path=HISTORY_PATH):
    raw = path.read_bytes()
    assert hashlib.sha256(raw).hexdigest() == HISTORY_SHA256, "historical fixture digest mismatch"
    history = json.loads(raw)
    assert history["schema_version"] == 1
    commits = history["commits"]
    expected = {
        "legacy_donor": "97f2881b0e5f43b7e9563569d92dfe702ed1df0b",
        "stage9_donor": "30dbab28692793fa311c82ae639186003f3a67d9",
        "stage9_target": "93c8511ab187b97395a0a5b4160a47e7b7e412c3",
    }
    assert {role: entry["sha"] for role, entry in commits.items()} == expected
    for entry in commits.values():
        assert entry["chromium_version"] == "152.0.7977.82"
        for name, text in entry["files"].items():
            data = text.encode("utf-8")
            assert entry["inventory"][name] == {"mode": "100644", "sha256": hashlib.sha256(data).hexdigest()}
...
@pytest.mark.parametrize("mutation", ["patch", "inventory", "sha", "series"])
def test_historical_fixture_rejects_tampering(tmp_path, mutation):
    value = json.loads(HISTORY_PATH.read_bytes())
    target = value["commits"]["stage9_target"]
    if mutation == "patch":
        name = "patches/0192-display-native-emulated-transport.patch"
        target["files"][name] += "tampered\n"
        target["inventory"][name]["sha256"] = hashlib.sha256(target["files"][name].encode()).hexdigest()
    elif mutation == "inventory":
        target["inventory"].pop("patches/0001-base-BUILD-gn.patch")
    elif mutation == "sha":
        target["sha"] = "0" * 40
    else:
        target["files"]["patches/series"] += "patches/9999-unapproved.patch\n"
    path = tmp_path / HISTORY_PATH.name
    path.write_bytes((json.dumps(value, indent=2, ensure_ascii=True) + "\n").encode())
    with pytest.raises(AssertionError, match="historical fixture digest mismatch"):
        load_history(path)
...
class Fixture:
    def __init__(self, root, monkeypatch):
        ...
        versions = {
            "CHROMIUM_VERSION": migration.VERSION,
            "UNGOOGLED_VERSION": migration.VERSION + "-1",
            "UNGOOGLED_WINDOWS_VERSION": migration.VERSION + "-1.1",
        }
        for name in migration.PROTECTED_FILES:
            data = versions[name] + "\n" if name in versions else (ROOT / name).read_bytes().replace(b"\r\n", b"\n")
        ...
        put(self.previous, "build/ungoogled-revisions.psd1", "@{\n"
            f'  ChromiumVersion = "{migration.VERSION}"\n'
            f'  UngoogledVersion = "{migration.VERSION}-1"\n'
            f'  UngoogledCommit = "{core_sha}"\n'
            f'  UngoogledWindowsVersion = "{migration.VERSION}-1.1"\n'
            f'  UngoogledWindowsCommit = "{platform_sha}"\n' + "}\n")
        put(self.previous, "build/upstream-cache.json", migration.arp._json({
            "schema_version": 1, "chromium_version": migration.VERSION, "ungoogled_commit": core_sha,
            "sources": {"windows": {"chromium_version": migration.VERSION,
                                     "ungoogled_commit": core_sha, "head_sha": platform_sha}},
        }))
...
@pytest.mark.parametrize("kind,link", [
    (kind, link) for kind in ("source", "marker", "old-patch", "tooling", "asset", "out", "work")
    for link in (("symlink",) if kind == "work" else ("symlink", "hardlink"))
])
...
def test_historical_0152_roundtrip_on_hunk_complete_preimage(fx, history):
    old = history["legacy_donor"]["files"][migration.PATCH]
    new = history["stage9_donor"]["files"][migration.PATCH]
    assert hashlib.sha256(old).hexdigest() == migration.LEGACY_PATCH_SHA256
    assert old != new
...
@pytest.mark.parametrize("profile", [migration.LEGACY_PROFILE, migration.STAGE9_PROFILE])
@pytest.mark.parametrize("pin_file", ["CHROMIUM_VERSION", "build/ungoogled-revisions.psd1"])
def test_profiles_reject_non152_target_before_snapshot_access(fx, monkeypatch, profile, pin_file):
    path = fx.repo / pin_file
    path.write_bytes(path.read_bytes().replace(migration.VERSION.encode(), b"153.0.8010.36"))
    commit(fx.repo)
    before = snapshot(fx.work)

    def unexpected(*args, **kwargs):
        pytest.fail("unsupported target must be rejected before donor or snapshot validation")

    for name in ("_host_git", "_blockers", "_load_receipt", "_verify"):
        monkeypatch.setattr(migration, name, unexpected)
    sha = migration._profile_donor(migration._profile(profile))
    with pytest.raises(migration.arp.ApplyError, match="only Chromium 152.0.7977.82 Windows x64"):
        fx.run(sha=sha, profile=profile)
    assert snapshot(fx.work) == before
    assert not (fx.work / migration.BLOCKER).exists()


@pytest.mark.parametrize("profile", [migration.LEGACY_PROFILE, migration.STAGE9_PROFILE])
def test_main153_pins_reject_pinned152_profiles_without_a_snapshot(tmp_path, monkeypatch, profile):
    repo, work, previous = (tmp_path / name for name in ("repo", "work", "previous"))
    for name in ("CHROMIUM_VERSION", "build/ungoogled-revisions.psd1"):
        put(repo, name, (ROOT / name).read_bytes())
    assert (repo / "CHROMIUM_VERSION").read_text().strip() == "153.0.8010.36"
    monkeypatch.setattr(migration, "TRUSTED_REPO", repo)
    work.mkdir()
    previous.mkdir()
    sha = migration._profile_donor(migration._profile(profile))
    with pytest.raises(migration.arp.ApplyError, match="only Chromium 152.0.7977.82 Windows x64"):
        migration.migrate(work, previous, repo, sha, patch_bin=PATCH_BIN, profile=profile)
    assert list(work.iterdir()) == list(previous.iterdir()) == []


def test_historical_profile_pins_and_allowlists_are_unchanged(history):
    legacy = migration.MIGRATION_PROFILES[migration.LEGACY_PROFILE]
    stage9 = migration.MIGRATION_PROFILES[migration.STAGE9_PROFILE]
    assert migration.DONOR_SHA == legacy["donor_sha"] == "97f2881b0e5f43b7e9563569d92dfe702ed1df0b"
    assert migration.STAGE9_DONOR_SHA == stage9["donor_sha"] == "30dbab28692793fa311c82ae639186003f3a67d9"
    assert legacy["version"] == stage9["version"] == "152.0.7977.82"
    assert legacy["arch"] == stage9["arch"] == "x64"
    assert legacy["allowed_patches"] == {migration.PATCH}
    assert legacy["allowed_series_patches"] == ()
    assert legacy["accept_prior_receipt"] is False
    assert stage9["accept_prior_receipt"] is True
    assert stage9["prior_receipt_sha256"] == "cb7b483eaef7da43d1b125e296ff27067fd2fdc110e49c8b3c47852d6b147da9"
    donor, target = history["stage9_donor"], history["stage9_target"]
    assert donor["sha"] == stage9["donor_sha"]
    old_series = donor["files"]["patches/series"]
    new_series = target["files"]["patches/series"]
    additions = stage9["allowed_series_patches"]
    assert new_series == old_series + b"".join((name + "\n").encode() for name in additions)
    assert [int(Path(name).name[:4]) for name in additions] == list(range(192, 209))
    old_inventory, new_inventory = donor["inventory"], target["inventory"]
    changed = {name for name in old_inventory.keys() | new_inventory.keys()
               if old_inventory.get(name) != new_inventory.get(name)}
    assert changed - {"patches/series"} <= stage9["allowed_patches"]
    assert set(additions) == new_inventory.keys() - old_inventory.keys()
    assert old_inventory.keys() <= new_inventory.keys()
    assert stage9["allowed_patches"] == changed - {"patches/series"} | {CLOCK_PATCH}
    for name in stage9["allowed_patches"]:
        raw = target["files"][name]
        _, entries = migration.arp.transform_patch(raw, set(), [])
        assert entries == [(target, "modify", None) for target in stage9["patch_targets"][name]]
    for name in stage9["context_only_patches"]:
        old = donor["files"][name]
        new = target["files"][name]
        assert migration._patch_edits(old) == migration._patch_edits(new)
...
@pytest.mark.parametrize("mutation", ["valid", "reorder", "comment", "missing", "extra", "renumber", "multifile", "executable"])
def test_final_stage9_append_inventory_is_exact(stage9, monkeypatch, mutation):
    ...
    if mutation == "reorder":
        ...
        for number, name in enumerate(additions, 214):
            renamed = f"patches/{number:04d}-" + Path(name).name.split("-", 1)[1]
            (stage9.repo / name).rename(stage9.repo / renamed)
            series = series.replace(name.encode(), renamed.encode())
