...
def display_source(tmp_path_factory):
    ...
    for number in [125, 126, 127, 128, 170, *range(192, 214)]:
        ...
        if number == 170 or number >= 192:
            assert 'offset' not in (result.stdout + result.stderr).lower()
    return root
...
def test_f2_refresh_never_feeds_inherited_screen_to_native(display_source):
    ...
    assert 'web_preferences_ = preferences;\n  ApplyUxrWebPreferences(web_preferences_);' in view
    widget = source(display_source, FRAME + 'web_frame_widget_impl.cc')
