...
from tools.platform_pins import load_shared_pins
...
def test_independent_source_and_complete_predecessor_chains(tmp_path):
    ...
    pins = load_shared_pins(ROOT)
    assert EVIDENCE['chromium_version'] == pins['ChromiumVersion']
    assert EVIDENCE['core_commit'] == pins['UngoogledCommit']
